class BaseCommand {
  ...
  async execute(message, content, options = {}) {
    if (!message || !content) {
      ...
      this.increaseUsage()
      this.bot.log.command(this.name)
  ...
  cooldownCheck(user_id) {
    if (this.cooledDownUsers.hasOwnProperty(user_id)) {
      return false
    } else {
      this.cooledDownUsers[user_id] = true
      setTimeout(() => { delete this.cooledDownUsers[user_id] }, this.cooldown * 1000)
      return true
    }
  }
