    ...
    this.messageHandler = async msg => {
      ...
      if (prefix) {
        ...
        if (await this.bot.middleware.checkChannel(msg.channel, cmdTxt, args)) {
          ...
          if (!command.cooldownCheck(msg.author.id)) {
            command.sendMessage(msg.channel, "slow down pls")
            return
          }
...
**Example:** \`${this.options.defaultPrefixes[0]}${cmdTxt} derpibooru\`

${helpFields.map(f => `**${f.name}**: ${f.value}`).join('\n')}`)
      }
    }
    this.bot.log.command(cmdTxt)
  }
