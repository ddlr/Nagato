class MessageHandler {
			...
			if (prefix) {
				...
				if (await this.bot.middleware.checkChannel(msg.channel, cmdTxt, args)) {
					...
					if (await this.bot.middleware.checkCommand(msg.channel.guild, command.name)) {
						return
					} else if (this.permissonCheck(msg, command)) {
						return
					}
				...
				const cmdTxt = newContent.split(' ')[0].toLowerCase()
				const args = newContent.split(' ').slice(1).join(' ')
				if (this.bot.commands.has(cmdTxt) && !this.bot.commands.get(cmdTxt).needsPrefix) {
					let command = this.bot.commands.get(cmdTxt)
					command
						.process(msg, args)
						.catch(err => this.bot.log.error(err))
				} else {
	...
	help(message, cmdTxt, args) {
			if (this.bot.commands.size === 0) {
				...
				const help = this.bot.commands.filter(c => c.permissionsAll(message)).sort()
				...
				if (this.bot.help.helpFormat === 'embed') {
					message.channel.createMessage({
						embed: {
							...
							description: "Pass a specific command as a command argument to get additional help information with that specific command.",
							color: 0xC081C0,
							fields: helpFields
						}
					})
